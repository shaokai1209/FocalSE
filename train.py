...
def count_parameters(model, model_name="Model"):
    ...
    print(f"\n{model_name} 参数量统计:")
    print(f"  总参数量: {format_size(total_params)} ({total_params:,})")
    print(f"  可训练参数: {format_size(trainable_params)} ({trainable_params:,})")
    print(f"  不可训练参数: {format_size(non_trainable_params)} ({non_trainable_params:,})")
...
def train(rank, a, h):
    ...
    if rank == 0:
        ...
        print("="*100)
        enc_total, _, _ = count_parameters(encoder, "Encoder")
        qua_total, _, _ = count_parameters(quantizer, "Quantizer")
        gen_total, _, _ = count_parameters(generator, "Generator")
        total_all = enc_total + qua_total + gen_total
        
        print(f"所有模型总参数量汇总: {total_all:,} 参数 ({total_all/1024/1024:.2f} M)")
        print(f"- Encoder: {enc_total/1024/1024:.2f} M")
        print(f"- Quantizer: {qua_total/1024/1024:.2f} M")
        print(f"- Generator: {gen_total/1024/1024:.2f} M")
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            c = encoder(y)
            # print("c.shape: ", c.shape) # torch.Size([19, 512, 50])
