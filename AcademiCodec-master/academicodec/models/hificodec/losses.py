# academicodec/models/hificodec/losses.py
import torch
...
class SemanticResidualLoss(nn.Module):
    ...
    def forward(self, clean_audio, noisy_audio, c_clean, enhanced_c):
        sem_clean = self.get_semantic_feat(clean_audio)
        min_len = min(sem_clean.size(-1), c_clean.size(-1))
        sem_clean = sem_clean[..., :min_len]
        c_clean = c_clean[..., :min_len]
        enhanced_c = enhanced_c[..., :min_len]

        residual_clean = c_clean - sem_clean
        residual_enhanced = enhanced_c - sem_clean
