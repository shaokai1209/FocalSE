...
def main():
    args = get_args()
    # 【新增/修改这部分】从环境变量获取 local_rank
    import os  # 确保文件开头已导入 os，若未导入则添加
    # 若使用分布式训练，从环境变量读取 local_rank；否则设为 0
    args.local_rank = int(os.environ.get('LOCAL_RANK', 0))
    
    if args.seed is not None or args.cudnn_deterministic:
        seed_everything(args.seed, args.cudnn_deterministic)
    args.ngpus_per_node = torch.cuda.device_count()
    main_worker(args.local_rank, args)  # 此处参数仍为 args.local_rank，无需改


def main_worker(local_rank, args):
    # 【新增验证】确保 local_rank 非负
    if local_rank < 0:
        raise ValueError(f"local_rank 不能为负数！当前值：{local_rank}，请检查环境变量 LOCAL_RANK")
    ...
    train_dataset =NSynthDataset(audio_path=args.train_data_path)
    logger.log_info('valid set')
    valid_dataset = NSynthDataset(audio_path=args.valid_data_path)
